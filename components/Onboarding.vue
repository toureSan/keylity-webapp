<template>
  <div class="flex min-h-screen bg-gray-50">
    ...
    <div class="hidden md:flex flex-col justify-center items-center w-1/2 bg-purple-300 border-r border-gray-100">
      <img src="@/assets/images/onbording.png" alt="Onboarding visuel" class="w-180 h-180 object-contain mb-8" />
      <div class="fluid-title font-bold text-purple-900">Bienvenue sur Keylity</div>
      <div class="mt-2  text-center text-white font-extrabold max-w-xs">Votre assistant pour simplifier toutes vos
        démarches administratives.</div>
    </div>
    <!-- Colonne droite : formulaire -->
    <div class="flex flex-1 flex-col justify-center items-left px-20 sm:px-20 md:px-15 bg-white">
      <!-- Stepper juste au-dessus du formulaire -->
      <h2 class="fluid-title font-bold ">À propos de vous</h2>
      <p class="fluid-subtitle text-gray-500 mb-10 max-w-xl">Nous avons besoins de quelque information afin de vous
        permettre de profiter de nos services.</p>
      <div class="flex items-center justify-left gap-4 mb-9 w-full">
        <template v-for="n in 3" :key="n">
          <div
            :class="['w-8 h-8 rounded-full flex items-center justify-center font-bold shadow-lg transition-all duration-300', step === n ? 'bg-purple-600 text-white scale-110 ring-4 ring-purple-200' : step > n ? 'bg-purple-300 text-white' : 'bg-gray-200 text-gray-400']">
            {{ n }}
          </div>
          <div v-if="n < 3" class="w-12 h-1 rounded transition-all duration-300"
            :class="step > n ? 'bg-purple-400' : 'bg-gray-200' "></div>
        </template>
      </div>
      <!-- Formulaire multistep -->
      <form @submit.prevent="handleNext" class="w-full max-w-lg animate-fade-in">
        <div v-if="step === 1">
          <div class="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4">
            <div class="relative group">
              <input v-model="form.genre" type="text" id="genre" class="input-fancy peer" placeholder="Genre " />
              <label for="genre" class="label-fancy">Genre</label>
              <span class="icon-input"><svg width="20" height="20" fill="none" stroke="currentColor" stroke-width="2"
                  viewBox="0 0 24 24">
            ...
            <div class="relative group">
              <input v-model="form.nationalite" type="text" id="nationalite" class="input-fancy peer"
            ...
            <div class="relative group">
              <input v-model="form.nom" type="text" id="nom" class="input-fancy peer" placeholder="Nom " />
            ...
            <div class="relative group">
              <input v-model="form.prenom" type="text" id="prenom" class="input-fancy peer" placeholder="Prénom " />
            ...
            <div class="relative group">
              <input v-model="form.naissance" type="date" id="naissance" class="input-fancy peer"
                placeholder="Date de naissance " />
              <label for="naissance" class="label-fancy">Date de naissance</label>
              <span class="icon-input"><svg width="20" height="20" fill="none" stroke="currentColor" stroke-width="2"
                  viewBox="0 0 24 24">
            ...
            <div class="relative group">
              <input v-model="form.npa" type="text" id="npa" class="input-fancy peer" placeholder="NPA/Localité " />
            ...
            <div class="relative group">
              <input v-model="form.avs" type="text" id="avs" class="input-fancy peer" placeholder="Numéro AVS " />
            ...
            <div class="relative group">
              <input v-model="form.profession" type="text" id="profession" class="input-fancy peer"
        ...
        <div v-else-if="step === 2">
          ...
          <div class="grid grid-cols-1 gap-y-4">
            <div class="relative group">
              <input v-model="form.adresse" type="text" id="adresse" class="input-fancy peer"
            ...
            <div class="relative group">
              <input v-model="form.telephone" type="tel" id="telephone" class="input-fancy peer"
            ...
            <div class="relative group">
              <input v-model="form.email" type="email" id="email" class="input-fancy peer" placeholder="Email " />
          ...
          <div class="flex flex-col items-left gap-4">
            <div class="relative group w-28 h-28 mb-2">
              ...
              <label for="photo"
                ...
                <span class="text-sm text-center mt-2 text-white font-semibold">Choisir une photo</span>
              </label>
            </div>
            <div class="w-full mt-2 relative group">
              <select v-model="form.etatCivil" id="etatCivil" class="input-fancy peer">
            ...
            <div class="w-full mt-2 relative group">
              <input v-model="form.domicilieDepuis" type="date" id="domicilieDepuis" class="input-fancy peer"
                placeholder="Domicilié depuis le " />
              <label for="domicilieDepuis" class="label-fancy">Domicilié depuis</label>
              <span class="icon-input"><svg width="20" height="20" fill="none" stroke="currentColor" stroke-width="2"
                  viewBox="0 0 24 24">
                  <rect x="3" y="4" width="18" height="18" rx="2" />
                  <path d="M16 2v4M8 2v4M3 10h18" />
            ...
            <div class="w-full mt-2">
              ...
              <div
                ...
                <div class="mb-1"><b>Nom :</b> {{ form.nom }} {{ form.prenom }}</div>
                ...
                <div class="mb-1"><b>Profession :</b> {{ form.profession }}</div>
                <div class="mb-1"><b>Nationalité :</b> {{ form.nationalite }}</div>
        ...
        <div class="flex justify-start mt-6 gap-2">
          <button v-if="step > 1" type="button" @click="step--"
            class="px-6 py-2 rounded bg-gray-100 text-gray-600 font-semibold hover:bg-gray-200 transition">Retour</button>
          <button type="submit"
