<template>
  <div class="bg-white rounded-xl shadow-sm">
    ...
    <div class="flex h-[calc(100vh-8rem)]">
          ...
          >
            <div class="flex items-center gap-3">
              ...
              <div class="flex-1 min-w-0">
                ...
                <p class="text-sm text-gray-500 truncate hidden md:block">{{ conversation.lastMessage }}</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <!-- Chat Area -->
      <div class="flex-1 flex flex-col bg-white" :class="{'hidden md:flex': !selectedConversation && isMobile}">
        <template v-if="selectedConversation">
          ...
          <div class="p-4 border-b border-gray-200 flex items-center gap-4">
            ...
            <div class="flex items-center gap-3 flex-1">
              ...
              <div>
                <h2 class="font-medium">{{ selectedConversation.name }}</h2>
            ...
            <div class="flex items-center gap-2">
              ...
              <button class="p-2 hover:bg-gray-100 rounded-lg">
                <Icon name="heroicons:video-camera" class="h-5 w-5 text-gray-500" />
              </button>
              <button class="p-2 hover:bg-gray-100 rounded-lg">
                <Icon name="heroicons:ellipsis-vertical" class="h-5 w-5 text-gray-500" />
              </button>
            </div>
          </div>
          ...
          <div class="flex-1 overflow-y-auto p-4 space-y-4">
            <template v-for="message in selectedConversation.messages" :key="message.id">
              ...
              >
                ...
                >
                <div 
                  class="max-w-[75%] md:max-w-md rounded-lg p-3"
                  :class="message.isSent ? 'bg-primary-500 text-white' : 'bg-gray-100'"
                >
                  <p>{{ message.content }}</p>
                  <span class="text-xs mt-1 block" :class="message.isSent ? 'text-primary-100' : 'text-gray-500'">
                    {{ message.time }}
                  </span>
                </div>
              </div>
            ...
            <div class="flex items-end gap-2">
              <div class="flex-1 relative">
                ...
                <div class="absolute right-2 bottom-2 flex items-center gap-2">
                  ...
                  <button class="p-2 hover:bg-gray-100 rounded-full">
                    <Icon name="heroicons:face-smile" class="h-5 w-5 text-gray-500" />
                  </button>
                </div>
              </div>
              <button 
        ...
        <div v-else class="flex-1 flex items-center justify-center">
          <div class="text-center">
            <div class="w-16 h-16 bg-gray-100 rounded-full flex items-center justify-center mx-auto mb-4">
              <Icon name="heroicons:chat-bubble-left-right" class="h-8 w-8 text-gray-400" />
            </div>
            <h3 class="text-lg font-medium text-gray-900">Vos messages</h3>
            <p class="text-gray-500 mt-1">Sélectionnez une conversation pour commencer</p>
          </div>
        </div>
      </div>
    </div>
